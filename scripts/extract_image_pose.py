...
class ArgumentParser(Tap):
    ...
    do_initial_extraction: bool = False
...
if __name__ == '__main__':
    ...
    for idx in range(len(forward_cam_rows)):
        forward_cam_rows[idx]["forward_image_idx"] = idx
    for idx in range(len(downward_cam_rows)):
        downward_cam_rows[idx]["downward_image_idx"] = idx
    ...
    '''
    forward propagate pose to timestamps that don't have it
    '''
    pose_keys = ['x', 'y', 'z', 'qx', 'qy', 'qz', 'qw']
    merged_df[pose_keys] = merged_df[pose_keys].fillna(method='ffill')

    '''
    extract out image poses
    '''
    forward_img_df = merged_df[merged_df['forward_image_idx'].isna() == False]
    downward_img_df = merged_df[merged_df['downward_image_idx'].isna() == False]

    forward_img_df = forward_img_df.drop(['forward_image_idx', 'downward_image_idx'], axis=1)
    downward_img_df = downward_img_df.drop(['forward_image_idx', 'downward_image_idx'], axis=1)

    '''
    save dataframes
    '''
    save_dir = Path(args.bagfile).parent
    forward_img_df.to_pickle(str(save_dir / "forward_data.pkl"))
    downward_img_df.to_pickle(str(save_dir / "downward_data.pkl"))

    print(f"saved image pose dataframes to {save_dir}")